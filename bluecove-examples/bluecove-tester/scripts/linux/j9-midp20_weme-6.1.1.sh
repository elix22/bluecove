...
cp ${BLUECOVE_JAR} "${JAVAHOME}/lib/jclMidp20/ext/bluecove.jar"
cp ${BLUECOVE_GPL_JAR} "${JAVAHOME}/lib/jclMidp20/ext/bluecove-gpl.jar"
cp ${BLUECOVE_GPL_PROJECT_HOME}/src/main/resources/*.so "${JAVA_HOME}/bin"


${JAVAHOME}/bin/emulator ${JVM_ARGS} -cp ${BLUECOVE_TESTER_HOME}/target/bctest.jar javax.microedition.lcdui.AppManager ${BLUECOVE_TESTER_HOME}/target/bctest.jad
rc=$?
if [ ! "${rc}" = "0" ]; then
