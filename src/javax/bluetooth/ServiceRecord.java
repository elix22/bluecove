...
public interface ServiceRecord {
	// these attributes should probably be removed. The standard doesn't 
	// include them
	...
	/**
	 * Returns a {@link java.lang.String} including optional parameters that can be used by a
	 * client to connect to the service described by this {@code ServiceRecord}. The
	 * return value can be used as the first argument to 
	 ...
	 * this {@code ServiceRecord}, "3" is the RFCOMM server channel mentioned in this
	 * {@code ServiceRecord}, and there are three optional parameters related to
	 * security and master/slave roles. If this method is called on a
	 * {@code ServiceRecord} returned from 
	 * {@link javax.bluetooth.LocalDevice#getRecord(javax.microedition.io.Connection)},
	 * it will return the connection string that a remote device will use to connect to this
	 * service.
	 * 
	 * @param requiredSecurity  determines whether authentication or
	 * encryption are required for a connection 
	 * @param mustBeMaster  {@code true} indicates
	 * that this device must play the role of master in connections to this
	 * service; {@code false} indicates that the local device is willing to be either
	 * the master or the slave 
	 * @return a {@link java.lang.String} that can be used to connect to
	 * the service or {@code null} if the ProtocolDescriptorList in this {@code ServiceRecord}
	 * is not formatted according to the Bluetooth specification 
	 * @throws java.lang.IllegalArgumentException  if requiredSecurity is not one of the
	 * constants {@code NOAUTHENTICATE_NOENCRYPT}, {@code AUTHENTICATE_NOENCRYPT}, or
	 ...
	 * {@link #setDeviceServiceClasses(int)}
	 ...
	 * {@link javax.bluetooth.L2CAPConnectionNotifier#acceptAndOpen()} is
	 * invoked for the first time on the notifier associated
	 * with this {@code ServiceRecord}, the classes argument from the
	 * {@link #setDeviceServiceClasses(int)} method is OR'ed with the current setting of the
	 ...
	 * calling {@link javax.bluetooth.LocalDevice#getDeviceClass()} on the 
	 * {@link javax.bluetooth.LocalDevice} object. Likewise, a call to
	 * {@link javax.bluetooth.LocalDevice#updateRecord(ServiceRecord)} will cause the 
	 * major service class bits to be
	 ...
	 * exception to this occurs if there is another {@code ServiceRecord} that is in the
	 * SDDB and {@link #setDeviceServiceClasses(int)} has been sent to that other
	 * {@code ServiceRecord} to request that some of the same bits be activated.
	 * 
	 * @param classes  an integer whose binary representation indicates
	 * the major service class bits that should be activated 
	 * @throws java.lang.IllegalArgumentException  if {@code classes} is not an
	 ...
	 * @throws java.lang.RuntimeException - if the {@code ServiceRecord}
	...
	/**
	 * Modifies this {@code ServiceRecord} to contain the service attribute defined by
	 * the attribute-value pair ({@code attrID}, {@code attrValue}). If the attrID does not
	 * exist in the {@code ServiceRecord}, this attribute-value pair is added to this
	 * {@code ServiceRecord} object. If the {@code attrID} is already in this {@code ServiceRecord}, the
	 * value of the attribute is changed to {@code attrValue}. If {@code attrValue} is null, the
	 * attribute with the attribute ID of {@code attrID} is removed from this
	 * {@code ServiceRecord} object. If attrValue is null and attrID does not exist in
	 ...
	 * {@code ServiceRecord} to the SDDB for the first time, or a call must be made to
	 * the updateRecord() method of LocalDevice to modify the version of this
	 * {@code ServiceRecord} that is already in the SDDB.
	 * 
	 * This method prevents the {@code ServiceRecord}Handle from being modified by
	 * throwing an IllegalArgumentException.
	 * 
	 * @param  attrID  the service attribute ID 
	 * @param attrValue  the {@link javax.bluetooth.DataElement}
	 * which is the value of the service attribute 
	 * @return {@code true} if the service
	 * attribute was successfully added, removed, or modified; {@code false} if
	 * {@code attrValue} is {@code null} and {@code attrID} is not in this object 
	 * @throws java.lang.IllegalArgumentException if {@code attrID} does not represent a 16-bit unsigned
	 * integer; if {@code attrID} is the value of {@code ServiceRecord}Handle (0x0000)
